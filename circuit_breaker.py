...
class CircuitBreaker:
    def __init__(self, func: Awaitable[T], exceptions, threshold: int, delay: int):
        """
    ...
    async def handle_closed_state(self, *args, **kwargs):
        allowed_exceptions = self.exceptions_to_catch
        try:
            ret_val = await self.func(*args, **kwargs)
    ...
    async def handle_open_state(self, *args, **kwargs):
    ...
    async def make_remote_call(self, *args, **kwargs):
        if self.state == StateChoices.CLOSED:
            return await self.handle_closed_state(*args, **kwargs)
        if self.state == StateChoices.OPEN:
            return await self.handle_open_state(*args, **kwargs)
...
class APICircuitBreaker:
    ...
    def __call__(self, func):
        ...
        async def decorator(*args, **kwargs):
            ret_val = await self.obj.make_remote_call(*args, **kwargs)
            return ret_val
